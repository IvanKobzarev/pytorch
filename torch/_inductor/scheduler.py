...
class FusedMixOrderReductions(FusedSchedulerNode):
    ...
    def fuse_with(self, other: BaseSchedulerNode, *, speculative: bool = False):
        ...
        if isinstance(other, FusedMixOrderReductions):
            fused_node1 = backend.fuse(
                self.node1, other.node1, speculative=speculative
            )
            fused_node2 = backend.fuse(
                self.node2, other.node2, speculative=speculative
            )
...
class FusedExternTritonKernelSchedulerNode(FusedSchedulerNode):
    ...
    def commit_epilogue_fusion(
        ...
        if mutated_name is None:
            ...
            # pyrefly: ignore[bad-assignment]
            mutated_name = node1.node.mutation_outputs[0].name
...
class Scheduler:
    ...
    def fusion_memory_timeline_peak_allowed_increase_bytes() -> int | None:
        ...
        if allowed_mb is None:
            return None
        return int(allowed_mb * 1024 * 1024)
