    ...
    for node in gm.graph.nodes:
        if node.op == "call_module" and node.target.startswith(prefix):
            ...
            with (
                ...
                # Disable compile-time autotuning for the regional AOT subcompile. Its
                # autotune block executes the region with real inputs, and when the region
                # contains nested real-call chunk boundaries that runs every chunk at once
                # (OOM). TODO: re-enable once the autotune harness respects the boundary.
                inductor_config.patch({"triton.autotune_at_compile_time": False}),
